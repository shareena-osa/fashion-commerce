...
const Cart = () => {
  ...
  return (
    <div className="box-decoration-clone bg-gradient-to-r from-indigo-100 to-pink-100 text-center animate-slide-in-left min-h-screen">
      <h2 className="mt-0 pt-16 font-bold text-5xl  pb-4 p-15">
        Your Shopping Cart
      </h2>
      <div className="cart-box mx-auto p-4 shadow-lg min-h-screen flex flex-wrap justify-center">
        {cartItems.length === 0 && <p>No items in the cart.</p>}
        {cartItems.map((item) => (
          <div
            key={item.id}
            className="cart-item flex flex-col items-start mb-2 mr-4"
          >
            <img
              src={item.image}
              alt={item.title}
